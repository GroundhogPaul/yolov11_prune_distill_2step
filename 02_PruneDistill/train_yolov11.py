...
def step2_constraint_train(sModel_step1_pretrained, sModelName_step2_constraint_trained, 
                           sYamlData, sYamlHyper):
    ...
    model.train(cfg = sYamlHyper, data=sYamlData, amp=False, save_period=3,
                name=sModelName_step2_constraint_trained)  # train the model

def step3_pruning(sModel_step2_constraint_trained, sModel_step3_pruned, pruning_rate=0.8):
    assert os.path.exists(sModel_step2_constraint_trained), f"Model path {sModel_step2_constraint_trained} does not exist."
...
def step4_finetune(sModel_step3_pruned, sName_step4_finetune, sDataYaml, sHyperYaml):
    ...
    model.train(data=sDataYaml, device="0", imgsz=640, epochs=200, batch=2, workers=0,
                name=sName_step4_finetune)  # train the model
...
#     model_s = add_attention(model_s)
    ...
    model_s.train(data=sYamlData, Distillation=model_t.model, loss_type='mgd', layers=layers, amp=False, imgsz=1280,
                  epochs=300,
                  batch=64, device=0, workers=0, lr0=0.001, name="Distillation")
...
if __name__ == '__main__':
    ...
    # ----- step3: pruning -----
    pruning_rate = 0.8
    sModel_step2_constraint_trained = os.path.join("./runs/LapaTrain", sModelName_step2_constraint_trained, "weights/best.pt")
