...
def start(fn_name, use_stack=True):
    """Start timing the specific function. Note: If use_stack is True, only one timer can be active at a time. Once you
    stop this timer, the previous one will start again.
    """
...
def stop(fn_name=None, use_stack=True):
    """If use_stack is True, this will stop the currently running timer and restore the previous timer on the stack if
    that exists. Note if use_stack is True, fn_name will be ignored.
    ...
    if use_stack:
        ...
        if _start_times[fn_name] > -1:
            ...
            print(f"Warning: timer for {fn_name} stopped before starting!")
...
def print_stats():
    ...
    all_fn_names = [k for k in _total_times if k not in _disabled_names]
...
class env:
    """A class that lets you go: with timer.env(fn_name): # (...) That automatically manages a timer start and stop for
    you.
    """
