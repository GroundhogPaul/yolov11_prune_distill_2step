...
class ResNetBackbone(nn.Module):
    ...
    def __init__(self, layers, atrous_layers=None, block=Bottleneck, norm_layer=nn.BatchNorm2d):
...
class ResNetBackboneGN(ResNetBackbone):
    ...
    def init_backbone(self, path):
        ...
        # strict=False because we may have extra uninitialized layers at this point
...
class DarkNetBackbone(nn.Module):
    ...
    def __init__(self, layers=None, block=DarkNetBlock):
        if layers is None:
            layers = [1, 2, 8, 8, 4]
        super().__init__()
...
class VGGBackbone(nn.Module):
    ...
    def __init__(self, cfg, extra_args=None, norm_layers=None):
    ...
    def _make_layer(self, cfg):
        """Each layer is a sequence of conv layers usually preceded by a max pooling. Adapted from
    ...
    def forward(self, x):
        ...
        for idx, layer in enumerate(self.layers):
            ...
            # Note that this differs from the original implementation
    ...
    def transform_key(self, k):
        ...
        return f"layers.{layerIdx}.{vals[1]}"
