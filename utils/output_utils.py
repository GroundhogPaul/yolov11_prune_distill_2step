...
def postprocess(
    ...
    """Postprocesses the output of Yolact on testing mode into a format that makes sense, accounting for all the
    ...
    if cfg.mask_type == mask_type.lincomb and cfg.eval_mask_branch:
        ...
        if cfg.preserve_aspect_ratio:
            # Undo padding
            masks = masks[
                :, : int(r_h / cfg.max_size * proto_data.size(1)), : int(r_w / cfg.max_size * proto_data.size(2))
            ]
...
def undo_image_transformation(img, w, h):
    """Takes a transformed image tensor and returns a numpy ndarray that is untransformed. Arguments w and h are the
...
def display_lincomb(proto_data, masks):
    ...
    for kdx in range(1):
        ...
        torch.sum(proto_data, -1).cpu().numpy()
