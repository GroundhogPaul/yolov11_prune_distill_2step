...
class RandomCrop:
    ...
    自定义实现图像与label随机裁剪相同的位置 没办法直接使用transform.resize() 因为是像素级别的标注，而resize会将这些标注变成小数.
    """
...
class image2label:
    """
    现在的标签是每张图都是黑色背景，白色边框标记物体，那么要怎么区分飞机和鸟等21类物体，我们需要将标签 改为背景是[0,0,0],飞机是[1,1,1],自行车是[2,2,2]...
