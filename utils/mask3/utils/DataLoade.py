...
class RandomCrop:
    """Crop the given PIL Image at a random location. 自定义实现图像与label随机裁剪相同的位置 没办法直接使用transform.resize()
    因为是像素级别的标注，而resize会将这些标注变成小数.
    """
...
class image2label:
    """现在的标签是每张图都是黑色背景，白色边框标记物体，那么要怎么区分飞机和鸟等21类物体，我们需要将标签 改为背景是[0,0,0],飞机是[1,1,1],自行车是[2,2,2]... voc classes =
    ...
    def __call__(self, image):
        ...
        #
        # # 筛选满足条件的位置：R=128, G=0, B=0
        # condition1 = (image[:, :, 0] == 128) & (image[:, :, 1] == 0) & (image[:, :, 2] == 0)
        # condition2 = (image[:, :, 0] == 0) & (image[:, :, 1] == 128) & (image[:, :, 2] == 0)
        # condition3 = (image[:, :, 0] == 128) & (image[:, :, 1] == 128) & (image[:, :, 2] == 0)
        #
        # # 将满足条件的位置映射到新数组上
        # new_array[condition1] = 1
        # new_array[condition2] = 2
        # new_array[condition3] = 3
        # b=np.array(new_array*50,dtype=np.uint8)
